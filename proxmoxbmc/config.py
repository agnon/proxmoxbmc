...
_CONFIG_FILE_PATHS = (
    os.environ.get('PROXMOXBMC_CONFIG', ''),
    os.path.join(os.path.expanduser('~'), '.pbmc', 'proxmoxbmc.conf'),
    '/etc/proxmoxbmc/proxmoxbmc.conf')
...
class ProxmoxBMCConfig(object):

    DEFAULTS = {
        'default': {
            'show_passwords': 'false',
            'config_dir': os.path.join(
                os.path.expanduser('~'), '.pbmc'
            ),
            'pid_file': os.path.join(
                os.path.expanduser('~'), '.pbmc', 'master.pid'
            ),
            'server_port': 50891,
